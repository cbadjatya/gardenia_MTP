...
private:
  ...
  int n_edges, *vertices, *reverse_vertices;
  VertexId max_degree;
  ...
  void fill_data(bool symmetrize, bool need_reverse, bool sorted, bool remove_redundents, int* source = NULL) {
    ...
    vertices = custom_alloc_global<int>(n_vertices+1);
    vertices[0] = 0;
    max_degree = 0;
  ...
  void build_reverse_graph() {
    ...
    reverse_vertices = custom_alloc_global<int>(n_vertices+1);
...
public:
  ...
  ~Graph() {
    ...
    if(map_vertices) {
      munmap(vertices, (n_vertices+1)*sizeof(int));
    } else {
  ...
  VertexSet N(VertexId vid) const {
    ...
    int begin = vertices[vid], end = vertices[vid+1];
    assert(begin >= 0);
    if(begin > end) {
  ...
  VertexId out_degree(VertexId v) { return vertices[v+1] - vertices[v]; }
	int edge_begin(VertexId v) { return vertices[v]; }
	int edge_end(VertexId v) { return vertices[v+1]; }
	VertexId getEdgeDst(int e) { return edges[e]; }
	VertexId get_max_degree() { return max_degree; }
  bool is_directed() { return directed; }
  bool has_reverse_graph() { return has_reverse; }
  int* out_rowptr() { return vertices; }
  VertexId* out_colidx() { return edges; }
  int* in_rowptr() { return reverse_vertices; }
  ...
  void orientation() {
    ...
    int *new_vertices = custom_alloc_global<int>(n_vertices+1);
    new_vertices[0] = 0;
    ...
    edges = new_edges;
    custom_free<int>(old_vertices, n_vertices);
...
inline int intersection_num(const VertexSet& a, const VertexSet& b) {
