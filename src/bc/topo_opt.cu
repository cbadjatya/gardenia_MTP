...
__global__ void preprocess1(int N, int *P, bool *isBad, uint64_t *CSR_N, int *numBadWarps)
{
    ...
    if (tid < N)
    {
        ...
        if (tid % 32 == 0 && max_value - min_value > 300) // value based on heuristics!
        {
            isBad[wid] = true;
...
__global__ void bc_update(int m, int *depths, bool *visited, int* tmap) {
	int id = blockIdx.x * blockDim.x + threadIdx.x;
	if (id < m) {
        // id = tmap[id];
...
void BCSolver(Graph &g, int source, ScoreT *h_scores) {
  ...
  VertexId *d_column_indices;
	...
	do {
		...
		bc_update <<<nblocks, nthreads>>> (m, d_depths, d_visited, thread_mappings);
