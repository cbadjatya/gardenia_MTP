...
#define BFS_VARIANT "topo_sort"


__global__ void set_neighbors(int m, int* num_neighbors, int* d_row_offsets){
    int id = blockIdx.x * blockDim.x + threadIdx.x;
    if(id < m){
        num_neighbors[id] = d_row_offsets[id + 1] - d_row_offsets[id];
    }
}
...
void BFSSolver(Graph &g, int source, DistT* h_dist, int magic) {
    ...
    cudaMalloc(&num_neighbors, m * sizeof(int));
    set_neighbors<<<nblocks, nthreads>>>(m, num_neighbors, d_row_offsets);
    thrust::sequence(thrust::device, tmap, tmap + m);
    thrust::sort_by_key(thrust::device, num_neighbors, num_neighbors + m, tmap);
	...
	do {
		...
		bfs_step <<<nblocks, nthreads>>> (m, d_row_offsets, d_column_indices, d_front, d_dist, iter, tmap);
