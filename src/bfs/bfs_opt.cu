...
__global__ void bfs_kernel(int m, int* row_offsets, int *column_indices, DistT *dist, bool *changed, int depth, int* tmap) {
...
void BFSSolver(Graph &g, int source, DistT* h_dist, int magic_val) {
    ...
    // for(int i=0;i<rmpd;i++){
    //     cout<<i<<" "<<hmap[i]<<"\n";
    // }
    // return;
    // Group Operations And Re-Remapping
    ...
    // h_ttg = (int*)malloc(numBadWarps*32);

    // h_lind = (int*)malloc(numBadWarps*32);
    // cudaMemcpy(h_ttg, threadToGroup, numBadWarps*32*sizeof(int), cudaMemcpyDeviceToHost);
    // cudaMemcpy(h_lind, localIndexInGroup, numBadWarps*32*sizeof(int), cudaMemcpyDeviceToHost);

   

    // h_group_load = (int*)malloc( h_grp_cnt * sizeof(int) );
    // h_group_map = (int*)malloc( h_grp_cnt * sizeof(int) );
    // h_group_size = (int*)malloc( h_grp_cnt * sizeof(int) );

    // cudaMemcpy( h_group_size, group_size, h_grp_cnt * sizeof(int), cudaMemcpyDeviceToHost);
    // cudaMemcpy( h_group_load, group_load, h_grp_cnt * sizeof(int), cudaMemcpyDeviceToHost);
    // cudaMemcpy( h_group_map, group_map, h_grp_cnt * sizeof(int), cudaMemcpyDeviceToHost);

    // for(int i = 0; i < h_grp_cnt; i++){
    //     cout<<i<<" "<<h_group_map[i]<<" "<<h_group_size[i]<<" "<<h_group_load[i]<<"\n";
    // }

    // return;


    // thrust sort functions
    // sort
    // cout<<h_grp_cnt<<"\n";
    ...
    // int* h_pref;
    // h_pref = (int*)malloc( h_grp_cnt * sizeof(int) );
    // cudaMemcpy( h_pref, pref, h_grp_cnt * sizeof(int), cudaMemcpyDeviceToHost);

    // int* h_rmap;
    // h_rmap = (int*)malloc( h_grp_cnt * sizeof(int) );
    // cudaMemcpy( h_rmap, r_map, h_grp_cnt * sizeof(int), cudaMemcpyDeviceToHost);
    ...
    // for(int i = 0; i < h_grp_cnt; i++){
    //     cout<<i<<" "<<h_rmap[i]<<" "<<h_group_size[i]<<" "<<h_group_load[i]<<" "<<h_pref[i]<<"\n";
    // }
    // cout<<endl;
    ...
    // for(int i=0;i<rmpd; i++){
    //     std::cout<<i<<" -> "<<h_row_offsets[hmap[i]+1] - h_row_offsets[hmap[i]]<<"\n";
    // }
    
    // std::cout<<std::endl;

	printf("\ttime for opt [%s] = %f ms.\n", BFS_VARIANT, elapsed);
